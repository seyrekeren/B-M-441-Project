...
export const signup = async (req: Request, res: Response) => {
    ...
    if (existingUser) {
        return res.status(500).json({message: 'Kullanıcı adı kullanılıyor'})
    }

    existingUser = await User.findOne({ email }).exec();

    if (existingUser) {
        return res.status(500).json({message: 'Bu mail kullanılmaktadır'})
    }
...
export const login = async (req: Request, res: Response) => {
    ...
    res.status(200).send({ severity: 'success', summary: 'Başarılı', detail: 'Başarıyla giriş yapıldı.', isSuccess: true });
};
